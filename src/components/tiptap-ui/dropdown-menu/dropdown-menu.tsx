  ...
  (
    ...
    const content = (
      <FloatingFocusManager context={context.context} modal={false} initialFocus={0} returnFocus={true}>
        <div
          ref={ref}
          className={`qt-editor-dropdown-menu ${className || ''}`}
          style={{
...
export const DropdownMenuItem = React.forwardRef<HTMLDivElement, DropdownMenuItemProps>(
  ({ children, disabled, asChild = false, onSelect, className, ...props }, ref) => {
    ...
    const itemProps: React.HTMLAttributes<HTMLDivElement> & {
      ...
      role: 'menuitem',
      className: `qt-editor-dropdown-menu-item ${className || ''}`,
...
export const DropdownMenuGroup = React.forwardRef<HTMLDivElement, DropdownMenuGroupProps>(
  ({ children, label, className, ...props }, ref) => {
    return (
      <div {...props} ref={ref} role='group' aria-label={label} className={`qt-editor-button-group ${className || ''}`}>
        {children}
      </div>
    );
  }
...
>(({ className, ...props }, ref) => (
  <Separator
    ref={ref}
    className={`qt-editor-dropdown-menu-separator ${className || ''}`}
