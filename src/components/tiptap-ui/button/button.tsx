...
export const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className = '', children, tooltip, showTooltip = true, shortcutKeys, 'aria-label': ariaLabel, ...props }, ref) => {
    ...
    if (!tooltip || !showTooltip) {
      return (
        <button className={`qt-editor-button ${className}`.trim()} ref={ref} aria-label={ariaLabel} {...props}>
          {children}
        </button>
      );
    }
    ...
    return (
      <Tooltip delay={200}>
        <TooltipTrigger className={`qt-editor-button ${className}`.trim()} ref={ref} aria-label={ariaLabel} {...props}>
          {children}
        </TooltipTrigger>
        <TooltipContent>
          {tooltip}
